...
class OpenM_SSO_AdminDAO_DBImpl extends OpenM_SSO_DAO_DBImpl implements OpenM_SSO_AdminDAO {
    ...
    public function remove($userId) {
        self::$db->request(OpenM_DB::delete($this->getTABLE(self::SSO_TABLE_NAME), array(self::USER_ID, $userId)));
    }

    public function get($userId = null) {
        if ($userId != null) {
            $array = array(self::USER_ID => $userId);
            return self::$db->request_HashtableString(OpenM_DB::select($this->getTABLE(self::SSO_TABLE_NAME), $array), self::USER_ID);
        }
        else
            return self::$db->request_HashtableString(OpenM_DB::select($this->getTABLE(self::SSO_TABLE_NAME)), self::USER_ID);
    }
